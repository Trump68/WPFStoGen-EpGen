...
public class hkaSkeleton
{
    ...
    public hkaBone FindBone(string findname, bool includehide = true)
    {
        hkaBone result = null;
        for (int i = 0; i < bones.Length; i++)
        {
            if (!includehide && bones[i].hide) continue;
            if (bones[i].name.ToLower() == findname.ToLower())
            {
                result = bones[i];
                break;
            }
    ...
    public int FindBoneIndex(string findname)
    {
        int result = -1;
        for (int i = 0; i < bones.Length; i++)
        {
...
public class hkaPose
{
    ...
    public bool Read(BinaryReader reader, int numTransforms, int numFloats)
    {
...
[Serializable]
public class hkaPoseData
{
    public int boneStart;
    public Dictionary<string,Transform> transformList;
}
...
public class hkaAnimation
{
    ...
    public void Read(BinaryReader reader)
    {
        ...
        for (int i = 0; i < numOriginalFrames; i++)
        {
            ...
            if (!this.pose[i].Read(reader, numTransforms, numFloats))
            {
                break;
            }
            readed++;
