...
int function StartSex(Actor[] Positions, sslBaseAnimation[] Anims, Actor Victim = none, ObjectReference CenterOn = none, bool AllowBed = true, string Hook = "")
	...
	return -1
endFunction
int function AB_StartSex(string abSceneName = "",int abSceneVariant = 0,Actor[] Positions,sslBaseAnimation[] Anims,Actor Victim = none,ObjectReference CenterOn = none,int restartStage, bool[] acycle,int[] erection,string[] AbFemaleExpressionList, int[] AbFemaleExpressionIndex,string[] AbMaleExpressionList, int[] AbMaleExpressionIndex)
	...
	Thread.AbMaleExpressionIndex = AbMaleExpressionIndex;
	Thread.AB_RestartStage = restartStage
	Thread.AB_Prepare()
	; Start the animation
	if Thread.StartThread()
		return Thread.tid
...
state Disabled
	sslThreadModel function NewThread(float TimeOut = 30.00)
